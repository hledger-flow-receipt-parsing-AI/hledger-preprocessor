...
# 2. Stitches cfg_1b5a + cat_basic + starting_journal + bank_csv +
#    receipt segment + journal_output into a full-path video for US-2b.3
...
def extract_field_markers_from_cast(events, raw_to_gif_fn):
    ...
    # category = 'cash:atm_withdrawal', first char 'c'
    ...
    # currency_index = '11', first char '1'
    curr_start = first_key_after('1', acct_enter) if acct_enter else None
    curr_enter = first_enter_after(curr_start) if curr_start else None
    if curr_enter:
        result[f'{prefix}__amount'] = raw_to_gif_fn(curr_enter)

    # amount = '100', first char '1'
    amt_start = first_key_after('1', curr_enter) if curr_enter else None
...
CSV_VIDEO="${GIFS_ROOT}/2b_data_files/output/bank_csv_atm_gbp.mp4"
RECEIPT_VIDEO="${OUTPUT_DIR}/2b_label_foreign_currency.mp4"
MATCH_VIDEO="${GIFS_ROOT}/3b_foreign_currency_match/output/3b_foreign_currency_match.mp4"
JRNL_VIDEO="${GIFS_ROOT}/2b_data_files/output/journal_output_atm_gbp.mp4"
FULL_PATH_VIDEO="${OUTPUT_DIR}/2b3_full_path.mp4"

ALL_SEGMENTS=("$CFG_VIDEO" "$CAT_VIDEO" "$STARTJ_VIDEO" "$CSV_VIDEO" "$RECEIPT_VIDEO" "$MATCH_VIDEO" "$JRNL_VIDEO")
MISSING=()
...
if [[ ${#MISSING[@]} -eq 0 ]]; then
    log "Stitching full-path video: cfg_1b5a + cat_basic + starting_journal + bank_csv_atm_gbp + foreign_currency_receipt + foreign_currency_match + journal_output_atm_gbp"
