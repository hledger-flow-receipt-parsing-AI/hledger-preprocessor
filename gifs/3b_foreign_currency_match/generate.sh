#!/usr/bin/env bash
# =============================================================================
# Foreign Currency Match Demo - GIF Generator
#
# Demonstrates US-3.2: Matching a GBP ATM receipt to an EUR bank CSV.
# Receipt: 100 GBP | CSV: 117.50 EUR (conversion rate 1.175)
#
...
DEMO_NAME="3b_foreign_currency_match"
...
header "Foreign Currency Match Demo Generator (using real CLI)"
...
asciinema rec "$CAST_FILE" \
    --command="$PYTHON -m gifs.automation.real_foreign_currency_match_demo" \
    --title "Step 3b: Foreign Currency Match (GBP → EUR)" \
    --idle-time-limit=2 \
    --rows 38 \
    --cols 100 \
    -y -q
...
echo "  with a GBP receipt against an EUR CSV transaction."
echo
echo "Add to README.md:"
echo "  ![Step 3b: Foreign Currency Match](gifs/3b_foreign_currency_match/output/${DEMO_NAME}.gif)"
echo
