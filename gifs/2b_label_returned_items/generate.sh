...
init_demo "2b_label_returned_items" "$@"

# ── Step 1: Record the segment-only returned-items receipt demo ───────
run_full_pipeline \
    "gifs.automation.real_label_returned_items_tui_demo" \
    "Step 2b: Label Receipt with Returned Items" \
    50 \
    120
...
gif_path = Path('${OUTPUT_DIR}/2b_label_returned_items.gif')
...
# ── Step 2: Stitch full-path video for US-2b.5 ───────────────────────
...
JRNL_VIDEO="${GIFS_ROOT}/2b_data_files/output/journal_output_return_net.mp4"
...
if [[ ${#MISSING[@]} -eq 0 ]]; then
    log "Stitching full-path video: cfg_1b1w + cat_basic + starting_journal + returned_items_receipt + journal_output_return_net"
