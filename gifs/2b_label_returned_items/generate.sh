...
# and 1 defective item was returned. Shows both net_bought_items and
# net_returned_items in the receipt label JSON.
#
...
DEMO_NAME="2b_label_returned_items"
...
header "Returned-Items Receipt Labelling Demo Generator"
...
asciinema rec "$CAST_FILE" \
    --command="$PYTHON -m gifs.automation.real_label_returned_items_demo" \
    --title "Step 2b: Label Receipt with Returned Items" \
    --idle-time-limit=2 \
    --rows 38 \
    --cols 100 \
    -y -q
...
echo
echo "Note: This demo shows labelling a receipt with bought and returned items."
echo "  Bought: 3 x 25 EUR = 75 EUR | Returned: 1 x 25 EUR = 25 EUR | Net: 50 EUR"
echo
