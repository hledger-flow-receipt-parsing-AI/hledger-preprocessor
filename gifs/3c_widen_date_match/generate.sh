#!/usr/bin/env bash
# =============================================================================
# Widen Date Match Demo - GIF Generator
#
# Demonstrates US-3.3: Receipt date Jan 15, CSV date Jan 18.
# Initial ±2 day margin misses → matching algorithm widens → match found.
#
...
header "Widen Date Match Demo Generator (using real CLI)"
...
asciinema rec "$CAST_FILE" \
    --command="$PYTHON -m gifs.automation.real_widen_date_demo" \
    --title "Step 3c: Widen Date Range (Delayed Posting)" \
    --idle-time-limit=2 \
    --rows 38 \
    --cols 100 \
    -y -q
...
echo "  with a receipt date 3 days before the CSV posting date."
echo
echo "Add to README.md:"
echo "  ![Step 3c: Widen Date Match](gifs/3c_widen_date_match/output/${DEMO_NAME}.gif)"
echo
