...
def is_legacy_account_block(obj: Any) -> bool:
    ...
    if not isinstance(obj, dict):
        return False
    keys = obj.keys()
    return "asset_category" in set(keys)
...
def replace_legacy_account(container: dict) -> dict:
    ...
    account_obj.pop("asset_category", None)
    account_obj.pop("asset_type", None)
    account_obj["base_currency"] = currency
    return {**container, "account": account_obj}


def deep_rename_key(obj: Any, old_key: str, new_key: str) -> Any:
    """
    Recursively rename every occurrence of old_key to new_key.
    ...
    if isinstance(obj, dict):
        ...
        for k, v in obj.items():
            if k == old_key:
                k = new_key
            result[k] = deep_rename_key(v, old_key, new_key)
        return result
    elif isinstance(obj, list):
        return [deep_rename_key(item, old_key, new_key) for item in obj]
    else:
...
def deep_replace_account(obj: Any) -> Any:
    ...
    if isinstance(obj, dict):
        ...
        if "account" in obj:
            try:
                obj = replace_legacy_account(obj)
            except (KeyError, ValueError, TypeError):
                # If currency missing or invalid → do NOT replace, keep original
                pass

        # Normal recursion into children (after possible replacement above)
        result = {}
        for k, v in obj.items():
