...
**As a** user running `./start.sh`,
**I want to** uncategorised bank CSV transactions to be detected and reported immediately — before the interactive matching step — via a dedicated `--check-categorisation` flag,
**so that** I don't waste time on manual receipt-to-CSV or CSV-to-CSV matching only to discover a categorisation error afterwards.
...
**As a** user running `./start.sh`,
**I want to** a clear summary of which receipt transactions are still unlinked to their CSV counterpart — shown immediately before the interactive matching step — via a dedicated `--check-matching` flag, and — if unlabelled receipt images exist — a hint that the missing match may be in one of them,
**so that** I see the full picture of what still needs matching before I enter the interactive flow, and I am reminded to label remaining receipt images if the missing counterpart might be there.
...
**As a** user who just finished labelling a receipt in the TUI,
**I want to** the system to automatically check whether the newly labelled receipt matches any CSV transaction as soon as I answer "no" to "want to add more accounts?",
**so that** I get immediate feedback on whether the receipt links to a bank transaction, without having to run the full pipeline again.

**Design note:** Deferred — TUI changes are complex and should be done one at a time. The US-4.7 implementation should be designed so this can be added later without restructuring.
...
**I want to** select a time period from the dashboard dropdown to filter the Sankey and Treemap plots,
...
**As a** user who wants to understand the spending pattern within a single category over time,
**I want to** click a treemap cell while in drill-down mode and see a time-series chart showing individual transactions, cumulative total, and period aggregations for that category,
**so that** I can spot trends, outliers, and seasonal patterns within a category.
...
**As a** user who scrolls through large dashboards with many plots,
**I want to** the toolbar (title, period selector, drill-down button, back button) to stay fixed at the top of the viewport, and keyboard shortcuts to toggle drill-down mode and go back,
**so that** I can always access controls without scrolling back to the top.
...
**As a** user who wants to view my finances at different time scales,
**I want to** choose between weekly, monthly, and yearly granularity, and have the period dropdown update accordingly,
**so that** I can view a single week, a single month, or an entire year of data.
...
**As a** user who drills down into a category with many subcategories,
**I want to** the legend to group subcategories hierarchically (e.g. all `groceries:*` entries under a "groceries" header) and let me toggle an entire group with a double-click while still toggling individual items with a single click,
**so that** I can quickly hide or show groups of related subcategories without clicking each one individually.
