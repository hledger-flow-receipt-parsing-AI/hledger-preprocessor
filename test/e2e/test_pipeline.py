"""E2E tests for the preprocessing pipeline.

These tests verify the core pipeline stages that --run-pipeline orchestrates:
...
External dependencies (conda, hledger-flow) are required for the full
pipeline test. Other tests focus on individual Python components.
"""
...
class TestRunPipeline:
    ...
    def test_run_pipeline_without_errors(
        ...
        """Test that --run-pipeline runs completely without errors.

        This test:
        1. Sets up a proper test environment
        2. Runs hledger_preprocessor --run-pipeline with test configuration
        ...
        try:
            ...
            print(stdout)
            if stderr:
                print("\n--- Pipeline STDERR ---", file=sys.stderr)
            ...
            if errors_found:
                ...
                for error in errors_found[:10]:
                    print(f"  • {error}")
                pytest.fail(
                    f"--run-pipeline had {len(errors_found)} error(s) in output"
                )

            print("\n✓ --run-pipeline completed with no errors")
            ...
            pytest.fail("--run-pipeline timed out after 900 seconds")
...
class TestPipelineGifGeneration:
    """Test GIF generation for the pipeline (Issue #142, scenario 1.1).
    ...
    def test_generate_pipeline_gif(
        self,
        temp_finance_root: Dict[str, Any],
        monkeypatch,
    ):
