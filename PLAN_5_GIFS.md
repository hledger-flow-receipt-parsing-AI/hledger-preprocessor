...
2. Show `config.yaml` with syntax highlighting (using Python `rich` library)
3. Highlight key sections with annotations:
...
2. Show `categories.yaml` with syntax highlighting
3. Highlight the tree structure:
   ```yaml
   groceries:
   ...
   repairs:
     bike: {}
   ```
4. Explain: "Categories become hledger accounts: `expenses:groceries:ekoplaza`"
...
2. Run `hledger_preprocessor --config config.yaml --edit-receipt`
3. TUI shows receipt image
4. Fill fields:
   - Date: 2025-01-15
   - Shop: Ekoplaza
...
5. Save → creates `groceries_ekoplaza_card.json`
...
2. Show inputs side-by-side:
   ...
   ```
3. Run `hledger_preprocessor --config config.yaml --link-receipts-to-transactions`
4. Show output: "Found 1 match → auto-linked (no TUI needed)"
5. Show updated receipt JSON now contains `csv_transaction` reference
...
2. **GIF 1** - `01_setup_config` (simple: display config with colors)
3. **GIF 2** - `02_add_category` (simple: display categories with colors)
4. **GIF 4** - `04_match_receipt_to_csv` (most novel: show matching logic)
5. **Rename existing** - `edit_receipt` → `03_label_receipt`, `start_sh_pipeline` → `05_run_pipeline`
6. **Write E2E test** - `test_gif_sequence.py`
